/**
 * SmartDose - ESP32 + DS3231 + Servos + Sensor IR de bandeja + Nextion
 * Framework: ESP-IDF / PlatformIO
 *
 * Cambio implementado:
 *  - Se RETIRA completamente el sensor IR superior/de almacenamiento.
 *  - Solo se usa el sensor IR inferior de bandeja.
 *  - Si al dispensar no se detecta pastilla en la bandeja en 3 segundos,
 *    el servo reintenta hasta 4 veces.
 *  - Si despues de los reintentos no se detecta pastilla, se marca error,
 *    se activa LED + buzzer, se muestra page3 y se registra en logs.
 *  - Si se detecta pastilla, se activa LED + buzzer hasta que la persona
 *    retire la pastilla de la bandeja.
 *
 * Comandos por monitor serial o Nextion:
 *  - T1=08:30:2              Tubo 1, 08:30, cantidad 2
 *  - T1=08:30:2;T2=08:30:1  Varios horarios
 *  - CFG,1,8,30,2           Formato recomendado desde Nextion
 *  - DISP1                  Dispensacion manual tubo 1
 *  - ACK1                   Confirmar error/alarma tubo 1
 *  - STATUS                 Estado del sistema
 *  - CLEAR                  Borrar horarios
 *  - REFRESH                Actualizar Nextion
 *  - SETTIME=17:43:00       Ajustar RTC
 */
...
static void nextion_actualizar_pantalla(void)
{
    ...
    for (int i = 0; i < MAX_TUBOS; i++) {
        ...
        bool bandeja = bandeja_tiene_pastilla(i);
        if (info.alerta_activa) alerta_general = true;
        if (info.estado == TUBO_FALLO) fallo_general = true;

        char obj[20];

        snprintf(txt, sizeof(txt), "T%d %s", i + 1, estado_a_texto(info.estado));
        snprintf(obj, sizeof(obj), "tT%dEst", i + 1);
        nextion_set_txt(obj, txt);

        snprintf(obj, sizeof(obj), "tT%dCont", i + 1);
        nextion_set_txt(obj, "No usado");

        snprintf(txt, sizeof(txt), "%s", bandeja ? "En bandeja" : "Sin dosis");
        snprintf(obj, sizeof(obj), "tT%dBand", i + 1);
        nextion_set_txt(obj, txt);
    ...
    if (fallo_general)       nextion_set_txt("tMsg", "Error de dispensacion. Revise el tubo.");
...
static void log_evento(int tubo, const char *nivel, const char *msg)
{
    ...
    printf("[%s] %02d:%02d:%02d | Tubo %d | %s\n", nivel, h, m, s, tubo + 1, msg);

    char txt[120];
    snprintf(txt, sizeof(txt), "Tubo %d: %s", tubo + 1, msg);
    nextion_set_txt("tMsg", txt);
}
...
static bool dispensar_con_reintentos_y_confirmar_bandeja(int idx, int cantidad)
{
    ...
    for (int intento = 1; intento <= MAX_REINTENTOS_CAIDA; intento++) {
        ...
        printf("[WARN] Tubo %d | No se detecto pastilla en bandeja. Reintento %d/%d\n",
               idx + 1, intento, MAX_REINTENTOS_CAIDA);
...
static void ciclo_dispensacion(int idx)
{
    ...
    if (!cayo) {
        char msg[120];
        snprintf(msg, sizeof(msg), "ERROR: Tubo %d no dispenso. Revisar atasco o recargar.", idx + 1);
        log_evento(idx, "ERROR", msg);
    ...
    log_evento(idx, "INFO", "Pastilla detectada en bandeja");
